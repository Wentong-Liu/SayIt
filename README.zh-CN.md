...
**在 macOS 26 及以上，SayIt 默认用苹果系统自带的语音引擎**（全新的设备端 SpeechAnalyzer）——**无需下载任何模型**，而且更快：转写几乎是瞬间完成，快到那句「转写中…」的提示根本来不及出现，松开键文字就已经落在光标处了。更老的 macOS（或你自己想用时）会回落到上面那套本地 WhisperKit 模型。
...
- **macOS 26+**：无需下载，语音模型由系统内置。
- **macOS 14–15**：本地 WhisperKit 模型约需 1–2 GB 磁盘，首次启动时自动下载一次。
...
在 **macOS 26 及以上**，SayIt 开箱即用：它使用苹果系统自带的语音模型，**无需下载任何东西**，第一次就能近乎即时地听写。在 **macOS 14–15**，SayIt 会在首次启动时**自动下载本地 WhisperKit 模型**（约 1–2 GB，仅一次）——可在 设置 和 菜单栏 看到进度——之后还有**一次性的准备过程**（模型为苹果神经引擎编译，首次约一两分钟）；准备好后一直很快。两种情况下都请在 系统设置 → 隐私与安全性 里授予**麦克风**和**辅助功能**——辅助功能用于监听全局热键、并把文字注入到其它 app（在 macOS 26 上还会被请求一次**语音识别**权限）。
