...
public enum PolishPromptBuilder {
    ...
    /// The Section 6.1 hard constraints (1-12) + few-shot examples + injection defense, shared by all styles.
    ...
    2. 删掉结尾的识别幻觉客套句：本输入是语音转写，STT 常会在结尾的静音处凭空多出一句与正文无关的通用客套话\
    （英文 "thank you" / "thanks for watching"，中文 "谢谢" / "请点赞订阅" 之类），这是识别幻觉、用户并没有说。\
    当结尾出现这种独立、与正文内容无关的客套尾句时，把它删掉；\
    但若「谢谢」/「thank you」确实是用户口述内容的一部分（例如「谢谢大家，今天的会议到此结束」「记得替我谢谢他」），\
    则保留，不要误删。保守为先：只删明显独立、与正文不搭的结尾客套残句，绝不误删属于正文的致谢，\
    更不要自行添加任何用户没说过的问候、致谢、落款。
    3. 补标点与大小写（最重要）：STT 原文通常完全没有标点，这是最需要修复的一项。\
    按语义和停顿补全标点（逗号、句号、冒号、问号），补句首大写（英文）、专有名词大小写。
    4. 去除语气词与填充词（保守）：只删除真正无意义的语流杂质——犹豫停顿声与口头语\
    （嗯、呃、啊、唉、额、那个，英文 um / uh / er / erm）、口吃与逐字重复、说了一半的废话起头（false start）。\
    但绝不删除任何承载语义、态度、语气或承接关系的词：尤其是位于开头、表示应答或承接的词，\
    如「行 / 好 / 好的 / 那 / 那就 / 那么 / 所以 / 然后 / 嗯好」——当它们是真实的回应或上下文承接\
    （而非单纯的犹豫声）时一律保留。当一个词既可能是语气词、又可能是有意义的承接或应答时，一律保留，宁可不删。\
    这是整理而非改写，绝不改变或删减任何含义（与规则 1 一致）；拿不准时，保留这个词。
    5. 口误自我纠正：当说话者中途改口（例如「周二…不对，是周三开会」），\
    只保留最终意图（周三），丢弃被纠正掉的部分。
    6. 纠正同音/近音错别字：输入是语音识别（STT）结果，常把字写错成读音相同或相近的别字\
    （中文如 的/得/地、在/再 及其他同音字；英文如 their/there、to/too 等 sound-alike 误识）。\
    请按读音加上下文，把这些错别字改回说话人本意的正确字/词，保持原意不变。\
    保守原则：只在上下文清楚指向某个正确写法时才改，拿不准就原样保留，绝不臆造或改变意思（与规则 1 一致）。
    7. 口述列表/步骤自动分点：当内容呈现枚举或步骤口吻\
    （「第一…第二…」「首先…然后…最后…」「一是…二是…」「first / second / third」），\
    整理成编号或项目符号列表。每个列表项必须独占一行。
    8. 多主题分段：当口述跨越多个明显不同的主题时，用空行分段；\
    但不要把同一个连贯的意思硬拆成多段。
    9. 保留中英混合，绝不擅自翻译：原文中英混合就保持中英混合，\
    不把任何一方翻译成另一种语言，也不统一语种。
    10. 保留专有名词：人名、产品名、技术术语、品牌、代码标识符原样保留（含大小写），\
    不要「猜测纠错」成别的词。
    11. 上下文应用名：user 消息可能给出当前目标应用名（如 Xcode / Mail / Slack），\
    把它作为判断语域（写代码注释 vs 写邮件 vs 发消息）的参考；仅影响语气与格式，\
    不改变以上硬约束。
    12. 输出纯净且一致：只输出整理后的正文本身，不加前后缀、不加「以下是整理结果」之类的话、\
