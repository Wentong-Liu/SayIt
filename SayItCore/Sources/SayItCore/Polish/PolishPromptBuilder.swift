...
public enum PolishPromptBuilder {
    ...
    /// The Section 6.1 hard constraints (1-11) + few-shot examples + injection defense, shared by all styles.
    ...
    6. 口述列表/步骤自动分点：当内容呈现枚举或步骤口吻\
    （「第一…第二…」「首先…然后…最后…」「一是…二是…」「first / second / third」），\
    整理成编号或项目符号列表。每个列表项必须独占一行。
    7. 多主题分段：当口述跨越多个明显不同的主题时，用空行分段；\
    但不要把同一个连贯的意思硬拆成多段。
    8. 保留中英混合，绝不擅自翻译：原文中英混合就保持中英混合，\
    不把任何一方翻译成另一种语言，也不统一语种。
    9. 保留专有名词：人名、产品名、技术术语、品牌、代码标识符原样保留（含大小写），\
    不要「猜测纠错」成别的词。
    10. 上下文应用名：user 消息可能给出当前目标应用名（如 Xcode / Mail / Slack），\
    把它作为判断语域（写代码注释 vs 写邮件 vs 发消息）的参考；仅影响语气与格式，\
    不改变以上硬约束。
    11. 输出纯净且一致：只输出整理后的正文本身，不加前后缀、不加「以下是整理结果」之类的话、\
